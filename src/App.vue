<template>
  <div class="layout">
    <header class="header">
      ...
      <!-- Add a button to open and collapse left aside to make page responsive -->
    ...
    <div v-if="isModalOpen && modalType === 'privacy'" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <p>
          Your privacy is important to us. We collect and use your personal information to provide
          services and improve our offerings. Please review our full policy for more details.
        </p>
      </div>
    </div>
    ...
    <div v-if="isModalOpen && modalType === 'terms'" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <p>
          By using our services, you agree to comply with our terms. Please read them carefully to
  ...
  computed: {
    filteredItems() {
      return this.selectedItems.filter((item) => {
        ...
        const matchesStatus =
          this.searchStatus === '' ||
          item.status.toLowerCase().includes(this.searchStatus.toLowerCase())
        const matchesBrand =
          this.searchBrand === '' ||
          item.brand.toLowerCase().includes(this.searchBrand.toLowerCase())
...
.footer {
  ...
  box-shadow:
    0 -5px 10px rgba(0, 0, 0, 0.2),
    0 -8px 20px rgba(0, 0, 0, 0.1); /* Slight shadow for separation */
}
...
/* logout and show all items button design */
...
/* making footer design aesthetic */
...
/* making it responsive for media width less than 768  pixel by changing whole layout */
...
/* for media devices witl less than 768 pixels it will show the left aside  and aleft aside will have a collapsibility a an icon in top left on clicking which it will accordingly on click shownin a sliding animation , and after clicking corresponding godown the slider will close and it will display the contents of clicked node in the main container */
